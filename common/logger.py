...
class AverageMeter:
    ...
    def __init__(self, dataset):
        ...
        if self.benchmark == 'pascal':
            ...
            self.class_ids_interest = torch.tensor(self.class_ids_interest).cuda()
            self.nclass = 1
        elif self.benchmark == 'road':
            self.class_ids_interest = dataset.class_ids
            self.class_ids_interest = torch.tensor(self.class_ids_interest).cuda()
            self.nclass = 1
        elif self.benchmark == 'deepgloberoad':
            self.class_ids_interest = dataset.class_ids
            self.class_ids_interest = torch.tensor(self.class_ids_interest).cuda()
            self.nclass = 1
            ...
            self.class_ids_interest = torch.tensor(self.class_ids_interest).cuda()
            self.nclass = 1
        elif self.benchmark == 'deepglobeforest':
            self.class_ids_interest = dataset.class_ids
            self.class_ids_interest = torch.tensor(self.class_ids_interest).cuda()
            self.nclass = 1
...
class Logger:
    ...
    def initialize(cls, args, training):
        ...
        # if os.path.isdir('logs/test_logs')==False:
        #     os.makedirs('logs/test_logs')
